#!/bin/bash
# ============================================================
# worktree.sh - Git Worktree 管理スクリプト
# ============================================================
# ガルパン・マルチエージェントシステム用
# 中隊ごとのワークツリーを管理する
#
# 使用例:
#   ./scripts/worktree.sh create platoon1 feature/login
#   ./scripts/worktree.sh list
#   ./scripts/worktree.sh switch platoon1 feature/signup
...
cmd_create() {
    ...
    if [[ -d "$worktree_path" ]]; then
        ...
        print_info "削除するには: ./scripts/worktree.sh cleanup $platoon"
        exit 1
    fi
    ...
    print_success "ワークツリーを作成しました: worktrees/$platoon ($branch)"
    echo ""
...
cmd_switch() {
    ...
    print_success "ブランチを切り替えました: $platoon -> $branch"
}
...
cmd_cleanup() {
    ...
    print_success "ワークツリーを削除しました: worktrees/$platoon"
...
show_help() {
    cat << 'EOF'
Git Worktree 管理スクリプト - ガルパン・マルチエージェントシステム

使用法:
  ./scripts/worktree.sh <command> [arguments]

コマンド:
  create <platoon> <branch>   ワークツリーを作成
                              例: create platoon1 feature/login

  list                        ワークツリー一覧を表示

  switch <platoon> <branch>   指定ワークツリーでブランチを切り替え
                              例: switch platoon1 feature/signup

  cleanup <platoon> [--force] ワークツリーを削除
                              例: cleanup platoon1
                              例: cleanup platoon1 --force

  status                      全ワークツリーの git status を表示
    ...
    case "$command" in
        create)
            if [[ $# -lt 3 ]]; then
                ...
                echo "使用法: ./scripts/worktree.sh create <platoon> <branch>"
                exit 1
            fi
            cmd_create "$2" "$3"
            ;;
        ...
        switch)
            if [[ $# -lt 3 ]]; then
                print_error "引数が不足しています"
                echo "使用法: ./scripts/worktree.sh switch <platoon> <branch>"
                exit 1
            fi
            cmd_switch "$2" "$3"
            ;;
        cleanup)
            if [[ $# -lt 2 ]]; then
                print_error "引数が不足しています"
                echo "使用法: ./scripts/worktree.sh cleanup <platoon> [--force]"
                exit 1
            fi
            cmd_cleanup "$2" "${3:-}"
            ;;
        ...
        *)
            ...
            echo "ヘルプを表示: ./scripts/worktree.sh help"
            exit 1
            ;;
    esac
}
